...
public class BlockManager {
  ...
  public void processMetricsReport(DatanodeRegistration nodeID,
                                   String blockPoolId,
                                   BlockMetricsAsLongs bReport) throws IOException {
        namesystem.writeLock();
        ...
        try {
         ...
         /* if (namesystem.isInStartupSafeMode() && node.numBlocks() > 0) {
          ...
          if (bReport.getNumberOfBlocks() == 0) return;

          BlockMetricsAsLongs.BlockReportIterator it = bReport.getBlockReportIterator();
          int count = 0; //because its fun to use counters to debug
          NameNode.stateChangeLog.info("TotalBlocks:" + bReport.getBlockMetricsListAsLongs()[0]);
